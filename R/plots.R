...
source("~/projects/tables2graphs/Rcode/corr.R")
year <- 2000
office <- "Vereador"
...
dim(coalitions)
office.now <- office
coalitions <- subset(coalitions,(office==office.now))
...
alld <- merge(dnow,dnow,by=exclude(c("party","row_names"),names(dnow)),all=TRUE)

alld$y <- 1
allpc <- recast(alld,party.x~party.y,measure.var="y",fun.aggregate=sum,id.var=exclude("y",names(alld)))
...
colnames(x) <- rownames(x) <- tolower(colnames(x))

plot.coal(x,reorder=TRUE,breaks=c(c(0,.05,.10,.2,.3,.4,.5)),z=as.matrix(allpc[,-1]))

stop()


pdf(width=11,height=8,file=spath("coalplot.pdf"))
plot.coal(x,reorder=TRUE,breaks=c(c(0,.05,.10,.2,.3,.4,.5)),z=as.matrix(allpc[,-1]))
dev.off()

allpc.s <- recast(alld,party.x~uf,measure.var="party.y",fun.aggregate=length)


allpc.s <- recast(alld,party.x+uf~variable,measure.var="party.y",fun.aggregate=length)

allpc.s$party.x <- reorder(allpc.s$party.x,-allpc.s$party.y,FUN=sum)
allpc.s$uf <- reorder(allpc.s$uf,-allpc.s$party.y,FUN=sum)

allpc.s$y <- (allpc.s$party.y)





lp <- levels(allpc.s$party.x)
ls <- levels(allpc.s$uf)
##roll calls

load(file="~/projects/BrazilianPolitics/trunk/rollcalls/camara/processed/rolls_bio.RData")

data.a$vote <- as.numeric(as.character(car::recode(data.a$vote,"'sim'=1;'nao'=0;else=NA")))

da <- with(data.a,(data.frame(name.bio,state,party,session,vote)))
da <- subset(da,session==51 & (!party%in%c("s.part.","president") & (!is.na(vote))))


da.c <- recast(da,state+party~variable,measure.var="session")

da.c <- within(da.c,{
    party <- factor(party,levels=lp)
    state <- factor(toupper(state),levels=ls)
})
da.c <- subset(da.c,(!is.na(state))&(!is.na(party)))


tmp1 <- allpc.s[,-4]
names(tmp1) <- c("party","state","count")
tmp1$source <- "Municipal coalition dyads\n(2000 elections)"
tmp2 <- da.c
names(tmp2) <- c("state","party","count")
tmp2$source <- "Roll calls in the\nCâmara dos Deputados\n(1999-2002)"
tmp <- merge(tmp1,tmp2,all=TRUE)

library(colorspace)

p <- ggplot(tmp, aes(x=state,y=party))
p <- p + geom_tile(aes(fill=log10(count)))
p <- p+scale_fill_gradientn("Log10\nof counts",colours=(rainbow_hcl(4)),breaks=c(0,1,2,3,4),limits=c(0,4))+opts(panel.grid.minor =theme_blank(),panel.grid.major=theme_blank(),panel.background=theme_rect(colour="white",fill=gray(.975))) +scale_y_discrete("")+scale_x_discrete("")
p.all <- p+ facet_wrap(~ source) 

pdf(file="ufplotAll.pdf",width=16,height=8)
print(p.all)
dev.off()


p <- ggplot(allpc.s, aes(x=uf,y=party.x))
p <- p + geom_tile(aes(fill=log10(y)))
p <- p+scale_fill_gradientn("Log10\nof counts",colours=(rainbow_hcl(4)),breaks=c(0,1,2,3,4),limits=c(0,4))+opts(panel.grid.minor =theme_blank(),panel.grid.major=theme_blank(),panel.background=theme_rect(colour="white",fill=gray(.975))) +scale_y_discrete("")+scale_x_discrete("")
p.coal <- p


p <- ggplot(da.c, aes(x=state,y=party))
p <- p + geom_tile(aes(fill=log10(session)))
p <- p+scale_fill_gradientn("Log10\nof counts",colours=rainbow_hcl(4),breaks=c(0,1,2,3,4),limits=c(0,4))+opts(panel.grid.minor =theme_blank(),panel.grid.major=theme_blank(),panel.background=theme_rect(colour="white",fill=gray(.975))) +scale_y_discrete("")+scale_x_discrete("")
p.rolls <- p

pdf(file="ufplot.pdf",width=9,height=8)
print(p.coal)
dev.off()

pdf(file="ufplotRC.pdf",width=9,height=8)
print(p.rolls)
dev.off()
...
##plot ideal points
##x.bugs <- coda2bugs(jags.star,parms.tokeep="^x")
...
b.bugs.m$Coefficient <- factor(b.bugs.m$X2,levels=c("b[1]","b[2]","b[3]"),labels=c("Intercept","Ln party size","Ln number of seats"))

m <- ggplot(b.bugs.m,aes(x=as.numeric(Coefficient),y=value))
m <- m+ stat_summary(fun.data=iqr,geom="pointrange")+scale_x_continuous(name="",breaks=c(1:3),labels=c("Intercept","Ln party size","Ln number of seats"),limits=c(.9,3.1),expand=c(0,0))+theme_bw3()+scale_y_continuous(name="",limits=c(-6.5,0),expand=c(0,0))
m <- m##+geom_line(data=data.frame(x=c(.9,3.1),y=c(0,0)),aes(x=x,y=y),colour="red",size=.6)
m <- m+coord_flip()
m
...
m <- m+ stat_summary(fun.data=iqr,geom="ribbon")+ scale_fill_hue(alpha = 0.5)
m <- m+facet_wrap(~party.size)+scale_y_continuous(limits=c(0,.5),expand=c(0,0),name="Probability of coalition formation")+scale_x_continuous(limits=c(0,2),name="Spatial distance")
...
## mdata <- data.frame(citystate=lmun,g=jagsfit$mean$g)
## tmp <- mun.data
## tmp$citystate <- with(tmp,paste(municipio,uf,sep=";"))
## mdata <- merge(mdata,tmp)

##load("coalitions2000partyVer.RData")
...
library(ggplot2)
## p <- qplot(mu.x,Mean.p,data=dv2,size=votes/1000,colour=I("red"),xlab="Ideology - electoral coalitions",ylab="Ideology - Camara (national) roll call votes")+geom_text(label=toupper(dv2$party),size=I(3),vjust=+2)+ theme_bw() + coord_equal()#+geom_smooth()
## print(p)
...
p <- p+scale_colour_brewer(palette="Set2",alpha=.7)
